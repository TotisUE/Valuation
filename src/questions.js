...
export const getQuestionsDataArray = () => {
    // Asegúrate de que la nueva clave exista en ScoringAreas importado
    ...
    const theQuestionsArray = [
        ...
        // === Section 4: Offering & Sales Effectiveness === (MODIFICADA)
        // --- Pregunta Mantenida 1 (off1) ---
        {
          id: "off1",
          section: sections[4], // Correcto, apunta al índice 4
          scoringArea: ScoringAreas.OFFERING_SALES, // <-- ACTUALIZADO
        ...
        // --- Pregunta Eliminada: off2 (Differentiation) ---
        // --- Pregunta Eliminada: off3 (Quality Systems) ---
        // --- Pregunta Eliminada: off4 (Innovation) ---

        // --- NUEVA PREGUNTA 4.2 (sales1) ---
        {
          id: "sales1",
          section: sections[4],
          scoringArea: ScoringAreas.OFFERING_SALES,
          text: "How effective and repeatable is your sales process from lead to close?",
          type: "mcq",
          valueKey: "salesProcessEffectiveness",
          options: [
            { text: "Highly effective: Well-documented sales process with clear stages, consistently followed, with conversion metrics tracked at each stage", score: 5 },
            { text: "Effective: Defined sales process exists, generally followed, with some metrics tracked", score: 4 },
            { text: "Somewhat effective: Basic sales process exists but is inconsistently followed or lacks clear stage definitions", score: 2 },
            { text: "Ad-hoc: No defined sales process; primarily relationship-based or opportunistic selling", score: 1 } // Correcto según issue (score 1)
          ],
          required: true
        },
        // --- NUEVA PREGUNTA 4.3 (sales2) ---
        {
          id: "sales2",
          section: sections[4],
          scoringArea: ScoringAreas.OFFERING_SALES,
          text: "How would you describe your typical sales cycle length and closing predictability?",
          type: "mcq",
          valueKey: "salesCyclePredictability",
          options: [
            { text: "Highly optimized: Short, predictable sales cycle relative to industry norms with reliable forecasting accuracy (>80%)", score: 3 },
            { text: "Moderately optimized: Average sales cycle length with reasonable forecasting accuracy (60-80%)", score: 2 },
            { text: "Somewhat challenging: Longer than desired sales cycles with limited forecasting accuracy (40-60%)", score: 1 },
            { text: "Highly unpredictable: Extended sales cycles with poor forecasting accuracy (<40%)", score: 0 }
          ],
          required: true
        },
        // --- NUEVA PREGUNTA 4.4 (sales3) ---
        {
          id: "sales3",
          section: sections[4],
          scoringArea: ScoringAreas.OFFERING_SALES,
          text: "How effectively does your business convert leads into qualified opportunities and closed sales?",
          type: "mcq",
          valueKey: "salesConversionRates",
          options: [
            { text: "Excellent conversion rates: Consistently above industry benchmarks at each funnel stage", score: 3 },
            { text: "Good conversion rates: Generally at or slightly above industry benchmarks", score: 2 },
            { text: "Average conversion rates: Meets minimum industry standards but room for improvement", score: 1 },
            { text: "Below average: Conversion rates consistently below industry benchmarks OR I don't know what the industry benchmarks are", score: 0 }
          ],
          required: true
        },
        // --- Pregunta Mantenida 2 (off5) ---
        {
          id: "off5",
          section: sections[4], // Correcto, apunta al índice 4
          scoringArea: ScoringAreas.OFFERING_SALES, // <-- ACTUALIZADO
        ...
        // === Section 8: Your Financials & Industry === (Sin cambios)
        ...
        { id: "finAdj", section: sections[8], text: "What are your typical annual EBITDA Add-backs / Adjustments?", type: "number", valueKey: "ebitdaAdjustments", placeholder: "e.g., $ 50,000 (can be 0)", required: false, helpText: "..." },
