...
export const getQuestionsDataArray = () => {
    ...
    const profileSectionName = sections[0]; // "Your Profile"
    const profileQuestions = getProfileQuestions(profileSectionName);

    const otherGeneratedQuestions = [
    ...
    const finalQuestionsArray = [...profileQuestions, ...otherGeneratedQuestions];

    console.log("--- Debugging getQuestionsDataArray ---");
    console.log("sections[0]:", sections[0]);
    console.log("profileQuestions from module (count):", profileQuestions ? profileQuestions.length : 0);
    if (profileQuestions && profileQuestions.length > 0) {
        console.log("First question in profileQuestions:", profileQuestions[0]);
        console.log("Does first profile question have correct sectionName?", profileQuestions[0].section === sections[0]);
    }
    console.log("otherGeneratedQuestions count:", otherGeneratedQuestions.length);
    console.log("Final questions array (count):", finalQuestionsArray.length);
    console.log("First 5 questions in final array:", finalQuestionsArray.slice(0, 5));
...
export const getQuestionsForStep = (stepIndex) => {
    if (!Array.isArray(sections) || stepIndex < 0 || stepIndex >= sections.length) {
        console.log("getQuestionsForStep: Invalid stepIndex or sections array not ready.");
        return [];
    }
    const sectionName = sections[stepIndex];
    console.log(`getQuestionsForStep: Getting questions for step ${stepIndex}, sectionName: "${sectionName}"`);

    const allQuestions = getQuestionsDataArray();
    if (!Array.isArray(allQuestions) || allQuestions.length === 0) {
         console.log("getQuestionsForStep: allQuestions is empty or not an array.");
         return [];
    }

    const questionsForThisStep = allQuestions.filter(q => q.section === sectionName);
    console.log(`getQuestionsForStep: Found ${questionsForThisStep.length} questions for section "${sectionName}".`);
    if (questionsForThisStep.length === 0) {
        console.log("getQuestionsForStep: No questions found. Checking sample of allQuestions sections:", allQuestions.slice(0,10).map(q => ({id: q.id, section: q.section})) );
    }
