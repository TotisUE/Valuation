// src/questions.js
import { ScoringAreas } from './scoringAreas'; // Asegúrate que ScoringAreas.OFFERING_SALES exista aquí
...
export const sections = [
    ...
    "Offering & Sales Effectiveness", // 4 <-- RENOMBRADO
...
export const getQuestionsDataArray = () => {
  ...
  const theQuestionsArray = [
      ...
      {
          id: "finRev", section: sections[8], text: "What is your approximate Last Full Year Revenue?", type: "number", valueKey: "currentRevenue", placeholder: "e.g., $ 1,500,000", required: true, helpText: "...",
          isEssentialForVC: true // <-- MODIFICACIÓN: Pregunta esencial
      },
      {
          id: "finGP", section: sections[8], text: "What is your approximate Last Full Year Gross Profit?", type: "number", valueKey: "grossProfit", placeholder: "e.g., $ 900,000", required: true, helpText: "...",
          isEssentialForVC: true // <-- MODIFICACIÓN: Pregunta esencial
      },
      {
          id: "finEBITDA", section: sections[8], text: "What is your approximate Last Full Year EBITDA?", type: "number", valueKey: "ebitda", placeholder: "e.g., $ 300,000", required: true, helpText: "...",
          isEssentialForVC: true // <-- MODIFICACIÓN: Pregunta esencial
      },
      { id: "finAdj", section: sections[8], text: "What are your typical annual EBITDA Add-backs / Adjustments?", type: "number", valueKey: "ebitdaAdjustments", placeholder: "e.g., $ 50,000 (can be 0)", required: false, helpText: "..." }, // No marcada como esencial
      {
          id: "industrySector", section: sections[8], text: "Select your primary Industry Sector:", type: "select", valueKey: "naicsSector", required: true,
          isEssentialForVC: true // <-- MODIFICACIÓN: Pregunta esencial
      },
      {
          id: "industrySubSector", section: sections[8], text: "Select your specific Industry Sub-Sector:", type: "select_dependent", dependsOn: "naicsSector", valueKey: "naicsSubSector", required: true,
          isEssentialForVC: true // <-- MODIFICACIÓN: Pregunta esencial
      },
  ];
...
// --- Helper Functions and Derived Data --- (Sin cambios aquí abajo)
...
// getQuestionsForStep (Usa getQuestionsDataArray)
...
// calculateMaxPossibleScore (Usa getQuestionsDataArray)
...
// calculateMaxScoreForArea (Usa getQuestionsDataArray)
...
export const ebitdaTiers = [   { threshold: 5000000, stageName: "Mature Scaleup", baseMultiple: 7.0, maxMultiple: 10.0 },
    ...
    // Añade una entrada para EBITDA negativo si quieres manejarlo explícitamente aquí,
    // aunque tu lógica actual lo maneja en el 'if (!tier)'
];

// --- getValuationParameters (SIN CAMBIOS) ---
export const getValuationParameters = (adjEbitda, sectorName, subSectorName) => {
    console.log("[getValuationParameters] START - adjEbitda:", adjEbitda, "sector:", sectorName, "subSector:", subSectorName); // LOG GVP 1

    const validAdjEbitda = typeof adjEbitda === 'number' && !isNaN(adjEbitda) ? adjEbitda : -1;
    console.log("[getValuationParameters] validAdjEbitda:", validAdjEbitda); // LOG GVP 2

    // Asegurarse que ebitdaTiers esté disponible
    if (!Array.isArray(ebitdaTiers) || ebitdaTiers.length === 0) { // <-- Añadido check de longitud
         console.error("[getValuationParameters] ERROR: ebitdaTiers no está disponible o está vacío.");
         return {}; // <-- Devolver objeto vacío explícitamente aquí para coincidir con el error
    }
    console.log("[getValuationParameters] ebitdaTiers:", ebitdaTiers); // LOG GVP 3

    const tier = ebitdaTiers.find(t => validAdjEbitda >= t.threshold);
    console.log("[getValuationParameters] Found tier:", tier); // LOG GVP 4

    if (!tier) {
         console.warn("[getValuationParameters] No tier found for adjEbitda. Returning default pre-revenue values."); // LOG GVP 5
         // Asegúrate que este objeto devuelto SÍ tenga 'stage'
         return { stage: "Pre-Revenue / Negative EBITDA", baseMultiple: 0, maxMultiple: 0, industryAdjustment: 1 };
    }

    console.log("[getValuationParameters] Calling getIndustryAdjustmentFactor..."); // LOG GVP 6
    let industryAdjustment; // Declarar fuera del try/catch
    try {
         if (typeof getIndustryAdjustmentFactor !== 'function') {
              ...
              industryAdjustment = 1; // Usar valor por defecto si la función no existe
         } else {
             industryAdjustment = getIndustryAdjustmentFactor(sectorName, subSectorName);
         }
         console.log("[getValuationParameters] industryAdjustment:", industryAdjustment); // LOG GVP 7

         // Verificar si el ajuste es un número válido
         if (typeof industryAdjustment !== 'number' || isNaN(industryAdjustment)) {
             console.warn("[getValuationParameters] industryAdjustment no es un número válido. Usando 1. Valor recibido:", industryAdjustment);
    ...
    // Verificar si tier.baseMultiple y tier.maxMultiple son números
    ...
    const maxMultiplier = typeof tier.maxMultiple === 'number' ? tier.maxMultiple : 0;
     if (typeof tier.baseMultiple !== 'number' || typeof tier.maxMultiple !== 'number') {
         console.warn("[getValuationParameters] tier.baseMultiple o tier.maxMultiple no son números. Tier:", tier);
     }


    const adjustedBaseMultiple = baseMultiplier * industryAdjustment;
    const adjustedMaxMultiple = maxMultiplier * industryAdjustment;
    console.log("[getValuationParameters] adjustedBaseMultiple:", adjustedBaseMultiple, "adjustedMaxMultiple:", adjustedMaxMultiple); // LOG GVP 8


    // Verificar tier.stageName antes de devolver
    const stageName = tier.stageName || "Unknown Stage"; // Valor por defecto si falta
    if (!tier.stageName) {
         console.warn("[getValuationParameters] tier.stageName no está definido en el tier encontrado. Tier:", tier);
    }
    ...
    console.log("[getValuationParameters] FINAL RESULT:", result); // LOG GVP 9
    return result;
};